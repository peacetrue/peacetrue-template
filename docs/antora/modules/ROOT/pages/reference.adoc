...
[source%nowrap,txt]
----
include::example$peacetrue-template-webflux/source-file-structure.txt[]
----
...
[source%nowrap,txt]
----
include::example$peacetrue-template-webflux/target-file-structure.txt[]
----

NOTE: WebFlux 没有 setting.gradle，使用时需要配合 <<_gradle_目录模板>>。

=== 变量说明

同 <<GradleRoot, Gradle 变量说明>>。
...
|名称
|字符串
|package.json 中 name，与 <<AntoraRepository>> 中 name 相同

|title
|标题
|字符串
|index.html 中 title，与 <<AntoraRepository>> 中 title 相同
|===
